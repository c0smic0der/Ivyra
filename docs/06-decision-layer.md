...
> **Positioning note — read this first.** This was written as an **additive feature, not a rename**: the plan going in was to leave the landing page, README, resume line, and the "a journal that scores your expectations against real outcomes" one-liner exactly as they were until this layer had been dogfooded for two to three weeks and had earned a repositioning. The decisions-only scope change broke that plan on its own terms — under decisions-only, the old copy no longer described a product that existed, and copy describing a product that no longer exists is worse than copy written slightly early. So the freeze was deliberately lifted, and marketing was realigned in the same build (`07-decision-layer-runbook.md`, Session 22 Prompt 4) to the new one-liner, "a decision journal that scores your expectations against real outcomes." The resume was updated separately and already carried the new line. The dogfooding gate itself still stands — it now governs whether the *wording* holds up to two to three weeks of real use, not whether to reposition at all; that question is already settled.
