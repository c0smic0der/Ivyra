# Building Marne with Claude — Understanding & Setup Guide
...
- `.` — "scaffold into the **current folder**" rather than creating a new subfolder. This is the one deviation from the generator's default flow, and it's what keeps everything directly inside `decision_calibrator` instead of nesting a second folder within it. The folder must be empty (or near-empty) for this to work — if the generator complains about conflicting files, list what's there (`ls -a`) and clear it out first. The generated app's internal name will be `decision_calibrator` (the folder name); the *product* is still called Marne — an npm package name and a product name are separate things, and only the latter appears in the UI.
...
*Everything here applies to any project, not just Marne. These are the fundamentals that separate people who fight the tool from people it multiplies. Explained from zero.*
