...
A user writes down a claim about the future — "we ship the redesign by the 15th." They attach a confidence (a number from 50 to 99 meaning "how sure I am this happens"), a resolution date (the day they'll know the answer), and optionally their reasoning in free text. When they save, the record **freezes**: the claim and reasoning can never be edited again. This is deliberate — the product's honesty depends on your past self being unable to quietly rewrite what it believed.

Time passes. The resolution date arrives. The user returns and answers one question: did it happen? Yes, no, or void (meaning the question stopped making sense — the project was cancelled, say). They can also write a note about what actually happened.
...
1. The user fills the capture form — claim, confidence, date, optionally reasoning and the plan/what-would-change-my-mind field — and hits save. The browser invokes the save **action** with the form's contents.
2. The action treats that input as hostile until proven otherwise (it came from an untrusted machine): it checks every field against the expected shape — confidence must be a whole number from 50 to 99, the claim must be non-empty and bounded, the date must be a date. Garbage is rejected here, before the database ever sees it.
...
4. The row is written. From this instant the claim and reasoning are frozen — no edit path exists anywhere in the application. The user sees their entry appear; the save is complete from their point of view.
5. *Then*, in the background, two pieces of enrichment run — background meaning the user never waits on them, and if they fail, the saved row is simply less decorated until a retry: the claim is sent to the language model with a tightly constrained instruction ("classify into exactly one of: work, health, money, relationships, self — answer with only the category"), and the answer, after validation against that closed list, fills the row's category; and the claim-plus-reasoning is sent to the embedding service, and the returned fingerprint is stored on the row (what that's for: next flow).
