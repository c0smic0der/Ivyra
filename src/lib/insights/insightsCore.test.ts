...
describe("buildInsightsViewModel — 40 resolutions", () => {
  ...
  it("progress.trend has one point per resolution and its last value matches last20", () => {
    expect(vm.progress.trend).toHaveLength(40);
    expect(vm.progress.trend[39]!.value).toBeCloseTo(vm.progress.last20!, 12);
  });

  it("curve.points and progress.trend are not reinvented — they match direct scoring calls", () => {
    const expectedPoints = calibrationBuckets(resolved).map((b) => ({
      x: b.meanConfidence,
      y: b.actualFrequency,
      n: b.n,
    }));
    expect(vm.curve.points).toEqual(expectedPoints);

    const expectedTrend = rollingBrierTrend(resolved, 20);
    expect(vm.progress.trend).toEqual(expectedTrend);
