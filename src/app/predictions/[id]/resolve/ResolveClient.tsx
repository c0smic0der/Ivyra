...
export function ResolveClient({ id }: { id: string }) {
  ...
  // fires this one Server Action, so the verdict and the (now full-length)
  // outcome note are written atomically.
  async function save() {
    if (verdict === null) return;
    setPending(true);
    const res = await resolvePrediction({ id, choice: verdict, outcomeNote: note });
    setResult(res);
    setPending(false);
